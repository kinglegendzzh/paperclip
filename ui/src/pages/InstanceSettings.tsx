...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.updateHeartbeatFailed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("common.error");
        throw new Error(
          failures.length === 1
            ? t("instance.disableOneFailed", { detail })
            : t("instance.disableManyFailed", { count: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.disableAllFailed"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instance.loadHeartbeatsFailed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instance.schedulerHeartbeats")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instance.heartbeatsDesc")}
        </p>
      </div>
