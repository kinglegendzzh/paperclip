  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("companyExport.failedToLoadData"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        ...
        body: t("companyExport.filesExportedAs", { count: resultCheckedFiles.size, filename: result.rootPath, s: resultCheckedFiles.size === 1 ? "" : "s" }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyExport.exportFailed"),
        body: err instanceof Error ? err.message : t("companyExport.failedToBuildPackage"),
      });
    },
  });
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {t("companyExport.exportTitle", { name: selectedCompany?.name ?? "Company" })}
            </span>
            <span className="text-muted-foreground">
              {t("companyExport.selectedFiles", { selectedCount, totalFiles, s: totalFiles === 1 ? "" : "s" })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyExport.warnings", { count: warnings.length, s: warnings.length === 1 ? "" : "s" })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ...
              : t("companyExport.exportCountFiles", { count: selectedCount, s: selectedCount === 1 ? "" : "s" })}
          </Button>
        </div>
      </div>
