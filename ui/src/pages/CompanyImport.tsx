      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyImport.binaryAssetPreviewNotAvailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.items", { count: conflicts.length, s: conflicts.length === 1 ? "" : "s" })}
          </span>
        </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.agents", { count: agents.length, s: agents.length === 1 ? "" : "s" })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
export function CompanyImport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("companySettings.orgChart"), href: "/org" },
      { label: t("companyImport.title") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t("companyImport.importCompleteBody", { company: result.company.name, count: result.agents.length, s: result.agents.length === 1 ? "" : "s" }),
      });
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("companyImport.localPackageInfo", { name: localPackage.name, count: Object.keys(localPackage.files).length, s: Object.keys(localPackage.files).length === 1 ? "" : "s" })}
                </span>
              )}
            </div>
            {!localPackage && (
              <p className="mt-2 text-xs text-muted-foreground">
                {t("companyImport.localZipHelpText")}
              </p>
            )}
          </div>
        ) : (
        ...
        <Field label={t("companyImport.targetLabel")} hint={t("companyImport.targetHint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
        ...
        {targetMode === "new" && (
          ...
          >
            <input
              ...
              placeholder={t("companyImport.newCompanyNamePlaceholder")}
            />
          </Field>
        )}
        ...
        >
          ...
          >
            <option value="rename">{t("companyImport.collisionRename")}</option>
            <option value="skip">{t("companyImport.collisionSkip")}</option>
            <option value="replace">{t("companyImport.collisionReplace")}</option>
          </select>
        </Field>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {t("companyImport.selectedFiles", { selectedCount, totalFiles, s: totalFiles === 1 ? "" : "s" })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyImport.conflicts", { count: conflicts.length, s: conflicts.length === 1 ? "" : "s" })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyImport.errors", { count: importPreview.errors.length, s: importPreview.errors.length === 1 ? "" : "s" })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("companyImport.importFilesCount", { count: selectedCount, s: selectedCount === 1 ? "" : "s" })}
            </Button>
          </div>
