...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("pluginSettings.settingsTitle"), href: "/instance/settings/heartbeats" },
      { label: t("pluginPage.plugins"), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("pluginSettings.title") },
    ]);
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("pluginSettings.dashboardDesc")}
                  </CardDescription>
                </CardHeader>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("pluginSettings.consecutiveCrashesTotal", { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettings.lastNLogEntries", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <CardContent>
                  {healthLoading ? (
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>{t("pluginSettings.healthChecksWait")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
 ...
 */
function formatUptime(uptimeMs: number | null, t: (key: string, options?: any) => string): string {
  if (uptimeMs == null) return "—";
  const totalSeconds = Math.floor(uptimeMs / 1000);
  if (totalSeconds < 60) return t("pluginSettings.time.seconds", { count: totalSeconds });
  const minutes = Math.floor(totalSeconds / 60);
  if (minutes < 60) return t("pluginSettings.time.minutesSeconds", { m: minutes, s: totalSeconds % 60 });
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return t("pluginSettings.time.hoursMinutes", { h: hours, m: minutes % 60 });
  const days = Math.floor(hours / 24);
  return t("pluginSettings.time.daysHours", { d: days, h: hours % 24 });
}
