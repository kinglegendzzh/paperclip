...
export function CompanySettings() {
  ...
  const inviteMutation = useMutation({
    ...
    onError: (err) => {
      setInviteError(
        err instanceof Error ? err.message : t("companySettings.createInviteFailed")
      );
    }
  });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <ToggleField
            label={t("companySettings.allowFeedbackSharing")}
            hint={t("companySettings.allowFeedbackSharingHint")}
            checked={!!selectedCompany.feedbackDataSharingEnabled}
            onChange={(enabled) => feedbackSharingMutation.mutate(enabled)}
          />
          <p className="text-sm text-muted-foreground">
            {t("companySettings.feedbackDesc")}
          </p>
            ...
            {selectedCompany.feedbackDataSharingConsentAt ? (
              <div>
                {selectedCompany.feedbackDataSharingConsentByUserId 
                  ? t("companySettings.enabledAtBy", { time: new Date(selectedCompany.feedbackDataSharingConsentAt).toLocaleString(), user: selectedCompany.feedbackDataSharingConsentByUserId })
                  : t("companySettings.enabledAt", { time: new Date(selectedCompany.feedbackDataSharingConsentAt).toLocaleString() })
                }
              </div>
            ) : (
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-border px-4 py-4">
          <p className="text-sm text-muted-foreground">
            <Trans
              t={t}
              i18nKey="companySettings.importExportMoved"
              components={{ 1: <a href="/org" className="underline hover:text-foreground" /> }}
            />
          </p>
            ...
            {archiveMutation.isError && (
              <span className="text-xs text-destructive">
                {archiveMutation.error instanceof Error
                  ...
                  : t("companySettings.archiveCompanyFailed")}
              </span>
            )}
          </div>
        </div>
