...
function getConcurrencyPolicyDescription(policy: string, t: TFunction): string {
  switch(policy) {
    case "coalesce_if_active": return t("routines.coalesceIfActiveDesc");
    case "always_enqueue": return t("routines.alwaysEnqueueDesc");
    case "skip_if_active": return t("routines.skipIfActiveDesc");
    default: return "";
  }
}

function getCatchUpPolicyDescription(policy: string, t: TFunction): string {
  switch(policy) {
    case "skip_missed": return t("routines.skipMissedDesc");
    case "enqueue_missed_with_cap": return t("routines.enqueueMissedWithCapDesc");
    default: return "";
  }
}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused") ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines.stateArchived") : t("routines.statePaused")}
            </span>
          ) : null}
        </div>
      ...
      <div className="flex items-center gap-3" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("routines.disableRoutineLabel", { title: routine.title }) : t("routines.enableRoutineLabel", { title: routine.title })}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines.stateArchivedCap") : enabled ? t("routines.stateOn") : t("routines.stateOff")}
          </span>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("routines.moreActionsLabel", { title: routine.title })}>
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines.titlePrefix") }]);
  }, [setBreadcrumbs]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toastRoutineCreated"),
        body: t("routines.toastRoutineCreatedBody"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toastFailedToUpdate"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toastFailedToUpdateGeneric"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toastRoutineRunFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toastRoutineRunFailedGeneric"),
        tone: "error",
      });
    },
  });
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.titlePrefix") },
